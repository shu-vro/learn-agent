...
Every source gets a **number**, cited inline as a markdown link immediately after the claim it supports:

- Web, fetched pages, YouTube, images: `[1](https://example.com/page)`
- Documents: `[2](reference_id=abc123:7)` — the `reference_id` copied exactly from the `[Source N]` block, with no backticks and no quotes
...
6. Never cite a bare number, a bare URL, or a bare `reference_id` — always the full `[n](target)` link form.
...
**End every answer with one combined `### Sources` list** — documents, web, YouTube, and images together, never split into subsections. It is a **numbered** list whose numbers match the inline citations exactly, in ascending order:

```markdown
### Sources

1. `reference_id=abc123:7` (page 3, score 0.35, type=text)
2. `reference_id=abc123:8` (page 3, score 0.33, type=text)
3. [Attention Is All You Need](https://arxiv.org/abs/1706.03762)
4. [Transformer architecture diagram](https://example.com/figure.png)
```

One entry per chunk (never merged) and one per page relied on. Document entries carry page, score, and type.
...
- Cite tool names, bare `[Source N]` labels, bare URLs, or `_Unnamed Document Chunk_` — every citation is a numbered `[n](target)` link
