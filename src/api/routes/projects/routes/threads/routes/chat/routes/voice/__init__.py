...
# Bump when the spoken text changes for the same message, so stored clips
# generated by the old rules are not replayed.
TTS_REVISION = "v3"
...
def voice_s3_key(user_id: str, message_id: str, voice_id: str) -> str:
    ...
    return f"{user_id}/{message_id}/{TTS_REVISION}-{voice_id}.mp3"
...
async def stream_voice(
    ...
    spoken = markdown_to_speech(message)
    if not spoken:
    ...
    return StreamingResponse(
        tts_stream(spoken, s3_key, voice),
